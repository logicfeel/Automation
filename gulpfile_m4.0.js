 ...
 * install-group
 */
gulp.task('install-group', function() {

    if (!CONFIG.options.intall_group)  return;
...
gulp.task('install-unit', function() {
    ...
    if (!CONFIG.options.intall_unit) return;
 ...
 * 템플릿
 */
gulp.task('template', gulp.parallel(['load-config', 'template-hbs']), function() {
 ...
 * 설치
 */
gulp.task('install', gulp.parallel(['load-config', 'install-group', 'install-unit', 'template-hbs']), function() {
...
gulp.task('clean-dist', function () {
    return gulp.src([PATH.base + PATH.dist, PATH.base + PATH.template.dist], {read: false, allowEmpty :  true})
      .pipe(clean());
});
...
// exports.init = gulp.parallel('init');
// exports.init = "aa";
 ...
 * preinstall 설정파일 구성
 */
gulp.task('preinstall', gulp.parallel('load-config', 'build-config-replace', 'save-config', function(cb) {
 ...
 * default 태스크
 */
gulp.task('default', gulp.parallel(['preinstall', 'install']));
